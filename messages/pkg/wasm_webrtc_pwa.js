/* @ts-self-types="./wasm_webrtc_pwa.d.ts" */

/**
 * @param {string} id
 * @returns {any}
 */
export function generate_outbound_metrics(id) {
    const ptr0 = passStringToWasm0(id, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.generate_outbound_metrics(ptr0, len0);
    return ret;
}
 ...
 * @returns {any}
 */
export function process_incoming_metrics(js_val) {
    const ret = wasm.process_incoming_metrics(js_val);
    return ret;
}
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('wasm_webrtc_pwa_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
